                ...
                <div class="media align-items-end profile-header">
                    ...
                    <div class="profile mr-3 mt-5">
                        @foreach($userData as $User)
                            @foreach($User->profile as $Profile) 
                                <img src="{{ asset('profileimg')}}/{{$Profile->profileimage}}" alt="Profile Picture" width="130" class="rounded mb-2 img-thumbnail">
                            @endforeach
                        @endforeach
                    ...
                    <div class="media-body mb-5 text-white" id="userdata">
                        ...
                        <ul class="list-unstyled user_data" >
                            @foreach($User->profile as $Profile) 
                            <li><i class="fa fa-map-marker user-profile-icon"></i> {{$Profile->address}}</li>
                            <li class="m-top-xs"><i class="fa fa-phone user-profile-icon"></i>
                            <a href="tel:" target="_blank">{{$Profile->contactnum}}</a>
                            </li>
                            @endforeach
                         </ul>
                        @endforeach
